...
public class ThymeleafTilesViewResolver extends ThymeleafViewResolver {
    ...
    public ThymeleafTilesViewResolver() {
        super();
    }
    ...
    protected View loadView(final String viewName, final Locale locale) throws Exception {
        ...
        ThymeleafTilesView view = BeanUtils.instantiateClass(ThymeleafTilesView.class);

        if (beanFactory.containsBean(viewName)) {
            final Class<?> viewBeanType = beanFactory.getType(viewName);
            if (ThymeleafTilesView.class.isAssignableFrom(viewBeanType)) {
                view = (ThymeleafTilesView) beanFactory.configureBean(view, viewName);
            } else {
                view = (ThymeleafTilesView) beanFactory.initializeBean(view, viewName);
            }
        } else {
            view = (ThymeleafTilesView) beanFactory.initializeBean(view, viewName);
        }
